...
**V2.5 Ejecución ciega de binarios externos:** ❌ **CRÍTICO — Supply chain risk.** El archivo `build-hoppscotch-agent.yml` descarga herramientas binarias precompiladas de repositorios de terceros (`tauri-apps/tauri`, `thedodd/trunk`) mediante `curl` y las ejecuta (`chmod +x`) sin validar su integridad. Bloques afectados: **líneas 85-87, 185-187, 293-300 y 385-392**; adicionalmente, en los jobs de Windows se descarga `trusted-signing-cli.exe` (`Levminer`) vía `Invoke-WebRequest` sin verificación en las **líneas 466 y 555**. Esto **contradice drásticamente su propio archivo `SECURITY.md`**, el cual hace un fuerte énfasis en la verificación estricta de firmas y hashes (BLAKE3) para proteger a los usuarios. Si un atacante compromete el release de origen, el pipeline ejecutaría malware con acceso a todos los secretos criptográficos de Apple y Azure del repositorio.
...
| C-4 | hoppscotch  | V2.5   | Ejecución ciega de binarios de terceros (`curl` + `tar`) sin validación de hash, contradiciendo su propio `SECURITY.md` | `build-hoppscotch-agent.yml:85-87, 293-300, 385-392, 466, 555`      |
...
| M-4 | gitea, librenms, mastodon, wiki, zoneminder | V3.3   | Sin archivo CODEOWNERS — sin revisión obligatoria por pares en áreas críticas                        |
...
```bash
# VULNERABLE (práctica actual, build-hoppscotch-agent.yml:85-87):
...
echo "<HASH-SHA256-ESPERADO>  cargo-tauri-x86_64-apple-darwin.zip" | sha256sum --check
unzip cargo-tauri-x86_64-apple-darwin.zip
chmod +x cargo-tauri
```
